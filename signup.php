...
<body>
    ...
    <div class="form_container" id="form_container">
        <form action="" method="post" id="form">
            <h2 align="center">Sign <span id="txt1">Up!</span></h2>
            <input type="text" name="userName" oninput="runanimation();" placeholder="Enter A User Name" value="<?php if(isset($_POST['userName'])) echo $_POST['userName']; ?>">
            ...
            <input type="password" name="pwd" id="" oninput="runanimation();" placeholder="Create Your Password" value="<?php if(isset($_POST['pwd'])) echo $_POST['pwd']; ?>">
            <input type="password" name="cPwd" id="" oninput="runanimation();" placeholder="Confirm Your Password" value="<?php if(isset($_POST['cPwd'])) echo $_POST['cPwd']; ?>">
            <p class="login" id="txt2">Already have an account? <a href="login.php">login from here</a>.</p>
    ...
    if(isset($_POST['sub'])){
        ...
        isset($_POST['cPwd']) and !empty($_POST['cPwd'])){
            ...
            if($pwd!==$cPwd) {
                echo "<script>alertMessage('Please check your password confirmation')</script>";
            } else {
                $hashed_password=password_hash($pwd,PASSWORD_DEFAULT);
                if(!filter_var($email,FILTER_VALIDATE_EMAIL)) {
                    echo "<script>alertMessage('Invalid email address')</script>";
                } else {
                    ...
                    if(!$conn) {
                        ...
                        if(mysqli_num_rows($emailCheck)>0) {
                            echo "<script>alertMessage('Email already exists')</script>";
                        } else {
                            ...
                            if($resul){
                                ...
                                echo "<script>alertMessage('Query error... Please come back later')</script>";
                            }
                        }
                    }
                }
            ...
            echo "<script>alertMessage('Make sure to input all fields please')</script>";
        }
    }
?>
    <!-- php end -->
